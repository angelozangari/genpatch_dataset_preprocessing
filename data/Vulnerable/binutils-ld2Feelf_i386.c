...
static bfd_boolean
gldelf_i386_check_ld_so_conf (const struct bfd_link_needed_list *l,
				       int force)
{
  ...
  if (! initialized)
      ...
      if (!gldelf_i386_parse_ld_so_conf (&info, tmppath))
	{
      ...
      if (info.path)
	{
	  ld_so_conf = gldelf_i386_add_sysroot (info.path);
	  free (info.path);
	}
      initialized = TRUE;
    }
...
gldelf_i386_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (path)
	    {
	      path = gldelf_i386_add_sysroot (path);
	      found = gldelf_i386_search_needed (path, &n, force);
	      free ((char *) path);
	      if (found)
		break;
	    }
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (path
		  && gldelf_i386_search_needed (path, &n, force))
		break;
	    }
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      path = gldelf_i386_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf_i386_search_needed (path, &n,
							      force));
	      free ((char *) path);
	    }
	  if (found)
	  ...
	  if (gldelf_i386_check_ld_so_conf (l, force))
	    break;
...
OUTPUT_ARCH(i386)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/i386-linux-gnu/lib32\"); SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/x86_64-linux-gnu/lib32\"); SEARCH_DIR(\"/media/sf_GitHub/binutils/analyze/tools/lib\"); SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/i386-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
