...
static bfd_boolean
gldelf_x86_64_check_ld_so_conf (const struct bfd_link_needed_list *l,
				       int force)
{
  ...
  if (! initialized)
      ...
      if (!gldelf_x86_64_parse_ld_so_conf (&info, tmppath))
	{
      ...
      if (info.path)
	{
	  ld_so_conf = gldelf_x86_64_add_sysroot (info.path);
	  free (info.path);
	}
      initialized = TRUE;
    }
...
gldelf_x86_64_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (path)
	    {
	      path = gldelf_x86_64_add_sysroot (path);
	      found = gldelf_x86_64_search_needed (path, &n, force);
	      free ((char *) path);
	      if (found)
		break;
	    }
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (path
		  && gldelf_x86_64_search_needed (path, &n, force))
		break;
	    }
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      path = gldelf_x86_64_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf_x86_64_search_needed (path, &n,
							      force));
	      free ((char *) path);
	    }
	  if (found)
	  ...
	  if (gldelf_x86_64_check_ld_so_conf (l, force))
	    break;
...
OUTPUT_ARCH(i386:x86-64)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/x86_64-linux-gnu/lib64\"); SEARCH_DIR(\"/media/sf_GitHub/binutils/analyze/tools/lib\"); SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/x86_64-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
