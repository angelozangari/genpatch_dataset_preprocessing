...
static char *
gldelf32_x86_64_add_sysroot (const char *path)
{
  int len, colons, i;
  char *ret, *p;
...
struct gldelf32_x86_64_ld_so_conf
{
  char *path;
  size_t len, alloc;
};
...
static bfd_boolean
gldelf32_x86_64_parse_ld_so_conf
     (struct gldelf32_x86_64_ld_so_conf *info, const char *filename);

static void
gldelf32_x86_64_parse_ld_so_conf_include
     (struct gldelf32_x86_64_ld_so_conf *info, const char *filename,
      const char *pattern)
{
  char *newp = NULL;
  ...
  if (glob (pattern, 0, NULL, &gl) == 0)
	...
	gldelf32_x86_64_parse_ld_so_conf (info, gl.gl_pathv[i]);
      globfree (&gl);
    }
#else
  /* If we do not have glob, treat the pattern as a literal filename.  */
  gldelf32_x86_64_parse_ld_so_conf (info, pattern);
#endif
      ...
      if (CONST_STRNEQ (p, "include") && (p[7] == ' ' || p[7] == '\t'))
	{
	  ...
	  do
		...
		++p;
	      ...
	      *p++ = '\0';
	      if (dir[0] != '\0')
		gldelf32_x86_64_parse_ld_so_conf_include (info, filename,
							       dir);
	    }
	  while (c != '\0');
	}
...
static bfd_boolean
gldelf32_x86_64_check_ld_so_conf (const struct bfd_link_needed_list *l,
				       int force)
{
  ...
  if (! initialized)
    {
      char *tmppath;
      struct gldelf32_x86_64_ld_so_conf info;

      info.path = NULL;
      info.len = info.alloc = 0;
      tmppath = concat (ld_sysroot, "/usr/local/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldelf32_x86_64_parse_ld_so_conf (&info, tmppath))
	{
	  free (tmppath);
	  tmppath = concat (ld_sysroot, "/etc/ld.so.conf",
			    (const char *) NULL);
	  gldelf32_x86_64_parse_ld_so_conf (&info, tmppath);
	}
      free (tmppath);

      if (info.path)
	{
	  char *d = gldelf32_x86_64_add_sysroot (info.path);
	  free (info.path);
	  ld_so_conf = d;
	}
      initialized = TRUE;
  ...
  needed.by = l->by;
  needed.name = l->name;
  return gldelf32_x86_64_search_needed (ld_so_conf, &needed, force);
}
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf32_x86_64_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  if (gldelf32_x86_64_search_needed (command_line.rpath,
						  &n, force))
	    break;
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (gldelf32_x86_64_search_needed (lib_path, &n,
						      force))
		break;
	    }
	  lib_path = (const char *) getenv ("LD_LIBRARY_PATH");
	  if (gldelf32_x86_64_search_needed (lib_path, &n, force))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
		...
		tmpname = gldelf32_x86_64_add_sysroot (tmpname);
	      found = (rp->by == l->by
		       && gldelf32_x86_64_search_needed (tmpname,
							      &n,
							      force));
	      if (tmpname != rp->name)
		free ((char *) tmpname);
	  ...
	  if (gldelf32_x86_64_check_ld_so_conf (l, force))
	    break;
...
OUTPUT_ARCH(i386:x64-32)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/usr/local/x86_64-pc-linux-gnu/libx32\"); SEARCH_DIR(\"/usr/local/libx32\"); SEARCH_DIR(\"/libx32\"); SEARCH_DIR(\"/usr/libx32\"); SEARCH_DIR(\"/usr/local/x86_64-pc-linux-gnu/lib\"); SEARCH_DIR(\"/usr/local/lib\"); SEARCH_DIR(\"/lib\"); SEARCH_DIR(\"/usr/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
