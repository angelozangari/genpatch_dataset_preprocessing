  ...
  if (! initialized)
    {
      ...
      tmppath = concat (ld_sysroot, "/media/sf_GitHub/binutils/analyze/tools/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldelf32_x86_64_parse_ld_so_conf (&info, tmppath))
	{
      ...
      if (info.path)
	{
	  ld_so_conf = gldelf32_x86_64_add_sysroot (info.path);
	  free (info.path);
	}
      initialized = TRUE;
    }
...
gldelf32_x86_64_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (path)
	    {
	      path = gldelf32_x86_64_add_sysroot (path);
	      found = gldelf32_x86_64_search_needed (path, &n, force);
	      free ((char *) path);
	      if (found)
		break;
	    }
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (path
		  && gldelf32_x86_64_search_needed (path, &n, force))
		break;
	    }
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      path = gldelf32_x86_64_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldelf32_x86_64_search_needed (path, &n,
							      force));
	      free ((char *) path);
	    }
	  if (found)
...
OUTPUT_ARCH(i386:x64-32)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/x86_64-linux-gnu/libx32\"); SEARCH_DIR(\"/media/sf_GitHub/binutils/analyze/tools/lib\"); SEARCH_DIR(\"=/media/sf_GitHub/binutils/analyze/tools/x86_64-linux-gnu/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
